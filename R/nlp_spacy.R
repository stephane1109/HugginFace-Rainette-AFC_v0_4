...
executer_spacy_filtrage <- function(ids, textes, pos_a_conserver, utiliser_lemmes, lower_input, modele_spacy, rv, strip_fr_elisions = FALSE, remove_numbers = FALSE) {
  ...
  args <- c(
    ...
    "--remove_numbers", ifelse(isTRUE(remove_numbers), "1", "0"),
...
executer_spacy_ner <- function(ids, textes, modele_spacy, rv, dictionnaire_json = NULL) {
  ...
  dico_ner_json <- ""
  if (!is.null(dictionnaire_json) && length(dictionnaire_json) > 0) {
    dico_ner_json <- trimws(as.character(dictionnaire_json[[1]]))
  }
