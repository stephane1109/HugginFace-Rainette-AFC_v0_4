...
# normalization for the non-symmetric case (in the case of some data)!  
	...
	factexpl<-sres$values[-1]
	#tot <- sum(sres$values[-1])
	#factexplp<-100*sres$values[-1]/tot
...
# In all cases, allow for first trivial first eigenvector.
	list(rproj=rproj[,-1],factexpl=factexpl)#,rcorr=rcorr[,-1], rcntr=rcntr[,-1],factexpl=factexpl,factexplp=factexplp, 
