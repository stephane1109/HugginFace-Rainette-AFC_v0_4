...
estimer_langue_corpus <- function(textes, rv = NULL, max_segments = 200) {
  if (is.null(textes) || length(textes) == 0) return(list(code = NA_character_, scores = c(fr = 0, en = 0, es = 0, it = 0, de = 0, pt = 0, ca = 0)))

  textes <- as.character(textes)
  textes <- textes[nzchar(trimws(textes))]
  if (length(textes) == 0) return(list(code = NA_character_, scores = c(fr = 0, en = 0, es = 0, it = 0, de = 0, pt = 0, ca = 0)))
  ...
  if (length(all_tokens) == 0) return(list(code = NA_character_, scores = c(fr = 0, en = 0, es = 0, it = 0, de = 0, pt = 0, ca = 0)))
...
configurer_langue_spacy <- function(langue) {
  ...
  if (!langue %in% c("fr", "en", "es", "it", "de", "pt", "ca")) langue <- "fr"
